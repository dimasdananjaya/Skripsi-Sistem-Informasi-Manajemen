...
<body>
    ...
    <!--<div id="app"> HAPUS NANTI ERRROR-->
        <nav class="navbar navbar-expand-md navbar-light navbar-laravel yesPrint">
          <div class="container">
            <!-- Brand/logo -->
            <img src="/img/lambangDinas.PNG" class="img-brand mr-auto" alt="logo">
            <h5 style="color:black;margin-left:10px;">
                ...
                <div class="collapse navbar-collapse noPrint" id="navbarSupportedContent">
                    <!-- Left Side Of Navbar -->
                    <ul class="navbar-nav mr-auto noPrint">
                    ...
                    <ul class="navbar-nav ml-auto noPrint">
                        ...
                        @else
                            <li class="nav-item dropdown noPrint">
                                <a id="navbarDropdown" class="nav-link dropdown-toggle noPrint" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    {{ Auth::user()->name }} <span class="caret"></span>
                                </a>

                                <div class="dropdown-menu dropdown-menu-right noPrint" aria-labelledby="navbarDropdown">
                                    <a class="dropdown-item noPrint" href="{{ url('petugas_uml/logout') }}"
                                       onclick="event.preventDefault();
                                                     document.getElementById('logout-form').submit();">
                                        {{ __('Logout') }}
                                    </a>

                                    <form id="logout-form" action="{{ route('petugas_uml_logout') }}" method="POST" style="display: none;">
                                        @csrf
                                    </form>

                                    <a class="dropdown-item noPrint" href="{{ url('/petugas_uml') }}">
                                        Dashboard
                                    </a>
                                </div>
                            </li>
